...
class _ExaminedPatientsPageState extends State<ExaminedPatientsPage> {
  ...
  Future<void> _loadAllExaminations() async {
    try {
      ...
      // جلب جميع الفحوصات من المصادر المختلفة
      ...
      // جمع كل الفحوصات في قائمة واحدة (بدون بيانات المرضى والأطباء بعد)
      final List<Map<String, dynamic>> allExaminations = [];
      final Set<String> patientIds = {};
      final Set<String> doctorIds = {};

      // Pass 1: nested examinations
      examinations.forEach((parentKey, patientNode) {
      ...
      // Pass 2: flat examinations
      examinations.forEach((key, value) {
        if (value is Map && value.containsKey('examinations')) return;
      ...
      // Pass 3: doctorExaminations
      doctorExams.forEach((examKey, examValue) {
        final examData = safeConvertMap(examValue);
        final String? patientId = examData['patientId']?.toString();
        final String? doctorId = examData['doctorId']?.toString();
        if (patientId == null || patientId.isEmpty) return;
        patientIds.add(patientId);
      ...
      // Pass 4: legacy examinations
      legacyExams.forEach((examKey, examValue) {
        final examData = safeConvertMap(examValue);
        final String? patientId = examData['patientId']?.toString();
        final String? doctorId = examData['doctorId']?.toString();
        if (patientId == null || patientId.isEmpty) return;
      ...
      // جلب بيانات المرضى دفعة واحدة
      final patientFutures = <Future>[];
      final Map<String, Map<String, dynamic>> patientsData = {};
      for (final id in patientIds) {
        patientFutures.add(_patientsRef.child(id).get().then((snap) {
          if (snap.exists) {
            final data = safeConvertMap(snap.value);
            data['id'] = id;
            patientsData[id] = data;
          } else {
            patientsData[id] = {'id': id, 'firstName': 'Unknown'};
          }
        }));
      }
      // جلب بيانات الأطباء دفعة واحدة
      final doctorFutures = <Future>[];
      final Map<String, Map<String, dynamic>> doctorsData = {};
      for (final id in doctorIds) {
        doctorFutures.add(_doctorsRef.child(id).get().then((snap) {
          if (snap.exists) {
            final data = safeConvertMap(snap.value);
            doctorsData[id] = data;
          } else {
      ...
      await Future.wait([...patientFutures, ...doctorFutures]);

      // بناء قائمة الفحوصات مع بيانات المرضى والأطباء
      final List<Map<String, dynamic>> allExamsWithData = allExaminations.map((exam) {
        final patient = patientsData[exam['patientId']] ?? {'id': exam['patientId'], 'firstName': 'Unknown'};
        final doctorId = exam['doctorId'];
        Map<String, dynamic> doctor = {'name': _translate(context, 'unknown')};
        if (doctorId != null && doctorId.isNotEmpty) {
          final d = doctorsData[doctorId];
          if (d != null) {
            doctor = {...d};
            doctor['name'] = d['fullName'] ?? _translate(context, 'unknown');
          }
        }
        return {
          'patient': patient,
      ...
      for (final exam in allExamsWithData) {
  ...
  // تحميل صورة اللوجو من الأصول (مسار نسبي صحيح)
  ...
  // تم حذف متغير xrayCorrectedClasses لأنه لم يعد مستخدمًا بعد حذف التحليل
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context ctx) => [
          ...
          // تم حذف التحليل من التقرير بناءً على طلب المستخدم
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        // تم حذف فقرة الأسنان المكتشفة باستخدام الذكاء الصناعي بناءً على طلب المستخدم
                    ],
                  ),
                );
              }).toList(),
